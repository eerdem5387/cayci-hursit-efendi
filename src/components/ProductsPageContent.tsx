...
export default function ProductsPageContent({ products, brands, brandParam, brand, filtered }: ProductsPageContentProps) {
  return (
    <motion.div
      ...
      animate="visible"
    >
      <motion.h1 variants={itemVariants} className="mb-6 text-2xl font-semibold text-gray-900">
        Ürünlerimiz
      </motion.h1>
      
      <motion.div variants={itemVariants} className="mb-6 flex flex-wrap items-center gap-2 text-sm">
        <span>Markaya göre filtrele:</span>
        <Link href="/urunlerimiz" className="rounded border border-gray-300 px-2 py-1 hover:border-emerald-600">
          Tümü
        </Link>
        {brands.map((b) => (
          <Link 
            ...
            className="rounded border border-gray-300 px-2 py-1 hover:border-emerald-600"
          >
            {b.name}
          </Link>
        ))}
